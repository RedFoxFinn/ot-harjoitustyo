...
class AddProduct:
  ...
  def _click_product(self):
    ...
    if len(value0) > 0 and len(value1) > 0 and len(value2) > 0:
      if len(value3) > 0:
        success = self._db.add_product(value0,value1,value2,value3)
      else:
  ...
  def _initialize(self):
    ...
    subtype_label = ttk.Label(master=self._frame, text=self.__texts[5])
    ...
    subtype_label.grid(row=4,column=0, padx=4, pady=4)
    self._subtype.grid(row=4,column=1, padx=4, pady=4)
    button.grid(row=5,column=1,padx=4, pady=4)